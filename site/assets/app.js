...
function apply(){
  ...
  const shown=cards.filter(c=>{
    const hay=`${c.name} ${c.tariff} ${c.description}`.toLowerCase();
    const zero=/0\s*₽/.test(c.price||'');
    return (!q||hay.includes(q))&&(!t||(c.types||[]).includes(t))&&(!p||(p==='zero'&&zero)||(p==='paid'&&!zero));
  });
...
async function load(){
  try{ const r=await fetch('/api/cards',{cache:'no-store',credentials:'same-origin'}); if(!r.ok) throw 0; cards=await r.json(); }
  catch{ try{ cards=await (await fetch('/assets/cards-default.json',{cache:'no-store'})).json(); }catch{ cards=[]; } }
  apply();
}
